...
if __name__ == '__main__':
    ...
    # Auto-detect: uses GPU when available (e.g. Colab), falls back to CPU
    # otherwise (e.g. local machine with no CUDA). Checkpoints were saved
    # from a CUDA run, so map_location must match wherever we actually put
    # the model/tensors below.
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f'Using device: {device}')

    # Forced to 0 (single-process loading): multiprocessing DataLoader workers
    # combined with PyTorch's own OpenMP thread pool deadlock at interpreter
    # shutdown on macOS (main process hangs in Py_FinalizeEx -> os.waitpid on
    # a worker that never reaps) -- observed directly, not theoretical. Videos
    # here are short enough that single-process loading costs little.
    num_workers = 0
